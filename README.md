...
# Extra changes
As per the author stated, this custom control can be optimized by caching some values to optimize performance. This reimplementation applies that idea, while not necessarily caching some data points, it instead caches the entire array after it has been generated, and the rendering values for the stars will only be redrawn when any of the fields that part of the rendering process (such as the margin, and star count) has been changed. Otherwise, calling Invalidate() will simply recolor the stars, making the OnMouseMove() and other methods to be at least somewhat faster.
